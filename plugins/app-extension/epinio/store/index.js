...
import { keyFieldFor } from '@/plugins/steve/normalize';
...
function EpinioFactory(namespace, baseUrl) {
  return {
    ...
    state() {
      return {
        // TODO: RC required for proxyFor
        config:       { namespace, baseUrl },
        types:        {},
        socket:       null,
        queue:        [],
        wantSocket:   false,
...
export default (config = {}) => {
  const namespace = EPINIO_PRODUCT_NAME;

  config.baseUrl = config.baseUrl || `/${ namespace }`;

  return function(store) {
    const inst = EpinioFactory(namespace, config.baseUrl);
